...
`define PCIE_BASE_SEQ_SV
  ...
  class pcie_base_seq extends uvm_sequence#(pcie_dllp_seq_item);
    ...
    // each derived sequence overrides this to declare which state it owns
    dl_state_t seq_start_state = DL_ACTIVE;
  ...
  task pcie_base_seq::send_fc_dllp(dllp_type_t pkt_type, fc_type_t fc_type, pcie_dllp_seq_item item);
      ...
      start_item(item);
          ...
          // Scale fields
          item.dllp[39:38] = cfg.fc_credits_vregister.hdr_scale  [fc_type];
